-- Soft-warn stays for name/phone/whatsapp; hard-unique identifiers return typed block instead of 23505.
...
begin
  ...
  if v_coverage_location_id is not null then
    ...
    select
      v_org_id,
      v_offering_id,
      case when l.kind = 'AIRPORT' then 'AIRPORT_EXPLICIT'::public.coverage_mode
           else 'CITY_OR_REGION'::public.coverage_mode end,
      l.id,
      false
