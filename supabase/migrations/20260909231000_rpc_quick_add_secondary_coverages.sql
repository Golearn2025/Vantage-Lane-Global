-- Quick Add: accept secondary_coverage_location_ids (airports beyond primary zone).
...
declare
  ...
  v_service_code text := coalesce(
    nullif(upper(trim(p_payload->>'service_code')), ''),
...
begin
  ...
  select id, code into v_svc_id, v_svc_code
  from public.service_types
  where code = v_service_code and is_active
  limit 1;

  if v_svc_id is null then
    raise exception 'service_code % is invalid or inactive', v_service_code
      using errcode = '22023';
  end if;
  ...
  for v_sec_id in
    select nullif(trim(x), '')::uuid
    from jsonb_array_elements_text(
      coalesce(p_payload->'secondary_coverage_location_ids', '[]'::jsonb)
    ) as t(x)
  loop
    if v_sec_id is null then
      continue;
    end if;
    if not exists (select 1 from public.locations l where l.id = v_sec_id and l.is_active) then
      raise exception 'secondary coverage location % not found', v_sec_id
        using errcode = '22023';
    end if;
  end loop;
  ...
  loop
    ...
    if v_coverage_location_id is not null and v_sec_id = v_coverage_location_id then
      continue;
    end if;
    if exists (
      select 1
      from public.offering_coverages oc
      where oc.offering_id = v_offering_id
        and oc.location_id = v_sec_id
    ...
    insert into public.offering_coverages (
      ...
      v_offering_id,
      case
        when v_loc_kind = 'AIRPORT' then 'AIRPORT_EXPLICIT'::public.coverage_mode
        else 'CITY_OR_REGION'::public.coverage_mode
      end,
      v_sec_id,
      false
    );
